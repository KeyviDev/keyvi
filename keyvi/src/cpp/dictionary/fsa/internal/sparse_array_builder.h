...
//#define STATE_WRITING_DEBUG
    ...
    for (i = 0; i < len; ++i) {
      typename UnpackedState<PersistenceT>::Transition e = unpacked_state[i];
...
template<>
inline void SparseArrayBuilder<SparseArrayPersistence<uint32_t>>::WriteTransition(
    size_t offset, unsigned char transitionId, uint64_t transitionPointer) {
 ...
 */
template<>
inline void SparseArrayBuilder<SparseArrayPersistence<uint16_t>>::WriteTransition(
    size_t offset, unsigned char transitionId, uint64_t transitionPointer) {
...
template<>
inline void SparseArrayBuilder<SparseArrayPersistence<uint32_t>>::WriteFinalTransition(
    const size_t offset, const uint64_t value) {
  WriteTransition(offset + FINAL_OFFSET_TRANSITION, FINAL_OFFSET_CODE, value);
}

template<>
inline void SparseArrayBuilder<SparseArrayPersistence<uint16_t>>::WriteFinalTransition(
    const size_t offset, const uint64_t value) {
