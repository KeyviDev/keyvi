...
#ifndef BIT_VECTOR_64_H_
#define BIT_VECTOR_64_H_
...
final {
   public:
    ...
    inline bool Get(const size_t bit) const {
      return (bits_[bit >> 6] & ((uint64_t) 1 << (bit & 63))) != 0;
    }
   ...
   private:
    ...
    inline uint64_t GetUnderlyingIntegerAtPosition(const size_t byte_position,
      ...
      if (byte_position + 1 < bits_.size()) {
        return (bits_[byte_position] >> bit_position)
            | (bits_[byte_position + 1] << (64 - bit_position));
      }
