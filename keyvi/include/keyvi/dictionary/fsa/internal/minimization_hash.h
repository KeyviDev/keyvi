...
class MinimizationHash final {
 public:
  struct MemoryLimitConfiguration final {
   public:
    MemoryLimitConfiguration(size_t memory_limit, int generations, int maximum_number_of_items_per_table)
  ...
  MemoryLimitConfiguration FindMemoryLimitConfiguration(size_t memory_limit, int min, int max) const {
    ...
    for (int i = min; i <= max; ++i) {
      ...
      for (unsigned int step = 3; step < kHashMaxSizeStep; ++step) {
        ...
        size_t item_forecast_for_hashtable =
            kHashSizeStepTable[step] + std::min(kHashSizeStepTable[step] >> 2, max_cookie_size_);
      ...
      int usage = (maximum_number_of_items + (maximum_number_of_items >> 2)) * sizeof(T) * i;
      if (usage > best_fit_memory_limit) {
        best_fit_memory_limit = usage;
        best_fit_generations = i;
        best_fit_maximum_number_of_items_per_table = static_cast<int>(maximum_number_of_items * kLoadFactor);
      }
    }
  ...
  int64_t GetMemoryUsage() const { return (memory_usage_); }

 private:
  /// magic constants definition of good hash table sizes
  const size_t kHashSizeStepTable[22] = {
