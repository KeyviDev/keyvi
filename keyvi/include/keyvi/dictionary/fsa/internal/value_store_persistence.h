 ...
 public:
  ...
  RawPointer(uint64_t offset, HashCodeTypeT hashcode, size_t length)
    ...
    if (length > std::numeric_limits<unsigned short>::max()) {
      length_ = std::numeric_limits<unsigned short>::max();
    }
  }
  ...
  void SetCookie(int value) { cookie_ = static_cast<unsigned short>(value); }
...
struct RawPointerForCompare final {
 public:
  ...
  bool operator==(const RawPointer<HashCodeTypeT>& l) const {
    ...
    if (length_l < std::numeric_limits<unsigned short>::max()) {
