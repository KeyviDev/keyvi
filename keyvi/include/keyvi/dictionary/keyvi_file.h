...
#ifndef KEYVI_DICTIONARY_KEYVI_FILE_H_
#define KEYVI_DICTIONARY_KEYVI_FILE_H_
...
class KeyViFile {
 ...
 public:
  explicit KeyViFile(const std::string& filename) : file_stream_(filename, std::ios::binary) {
    ...
    const bool compact_size = boost::lexical_cast<uint32_t>(sparse_array_properties.get<std::string>("version")) == 2;
    const size_t bucket_size = compact_size ? sizeof(uint16_t) : sizeof(uint32_t);
    const size_t array_size = boost::lexical_cast<size_t>(sparse_array_properties.get<std::string>("size"));
  ...
  ptree automataProperties() const { return automata_properties_; }
...
#endif  // KEYVI_DICTIONARY_KEYVI_FILE_H_
