...
def symlink_keyvi():
    ...
    if not os.path.exists(keyvi_cpp_link):
        try:
            if not os.path.exists(keyvi_cpp):
                os.makedirs(keyvi_cpp)

            os.symlink(os.path.abspath(keyvi_cpp_source), keyvi_cpp_link)

            shutil.copy("../CMakeLists.txt", os.path.join(keyvi_cpp, "CMakeLists.txt"))
            shutil.copytree(
                "../cmake_modules", os.path.join(keyvi_cpp, "cmake_modules")
            )
            ...
            os.remove(os.path.join(keyvi_cpp, "CMakeLists.txt"))
...
class CMakeBuild(build_ext):
    def build_extension(self, ext: CMakeExtension) -> None:
        ...
        # KEYVI_VERSION_INFO shows you how to pass a value into the C++ code
        # from Python.
        cmake_args = [
        ...
        cmake_args += [f"-DKEYVI_VERSION_INFO={self.distribution.get_version()}"]
